...
function add_object (type, object) { // {{{
	...
	if ( Schema['defined'] ) {
		if ( Schema.hasOwnProperty( type ) ) {
			...
			//
			logwrap.error( 'Object type ' + type + ' not defined in schema.' );
 ...
 * @param {object} object - the javascript object you wish to delete.
 */
function del_object (type, object) { // {{{
	...
	if (!serial) {
		logwrap.error( 'Failed to delete anonymous object' );
